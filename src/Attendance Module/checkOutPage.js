...
class App extends React.Component {
  ...
  SvgCircle = (props) => {
    ...
    return (
      <Svg height={circleHeight.height + 30} width={circleWidth.width - 30}>
        <Defs>
        ...
        <Rect height="100%" width="100%" fill="rgb(0, 0, 0)" mask="url(#mask)" fill-opacity="0" />
      </Svg>
    );
  }
...
async functionCombined() {
  ...
  if(allError){
    ...
    this.setState({Lat_LongError: true})
  ...
  back(){
   ...
   renderHeader(project){
    switch(project){
      ...
      case "2":
        return <PlainHeader title="Check Out" width={getWidthnHeight(40)} onPress={() => this.back()}/>
      }
    }
...
// const kayboard=this.state.KeyboardAvoidingView
  ...
  const {project, button} = this.props;
    //console.log("NEW PROPS: ", project, button, this.props.navigation)
    console.log("***CAMERA: ", this.props.file, this.state.image)
    let buttonColor = null;
    let gradient = null;
    let gradientShadow = null;
    if(project === "1"){
      buttonColor = ['#5434ff', '#21bbff'];
      gradient = button;
    ...
    const circleWidth = getWidthnHeight(50)
    const circleHeight = {height: circleWidth.width}
    var mapStyle = [{ "elementType": "geometry", "stylers": [ { "color": "#f5f5f5", "borderRadius": 30 } ] }]
    return (
