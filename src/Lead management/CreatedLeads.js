...
import {CommonModal, getWidthnHeight, LeadInfoModal} from '../KulbirComponents/common';
...
return (
        <View style={{height:viewportHeight,width:viewportWidth,backgroundColor:'#e6e6e6'}}>
          <Header titleFirstFloor='List Of Leads' titleGroundFloor='Created' onPress={() => {Actions.drawerOpen()}} source={require('../Image/newManuIcon.png')}
          ...
          {this.state.Lead_listing.map((item,key,event) => {
            console.log("DUE DATE CREATED: ", item)
             if (item.priority == '0'){
               var color = '#12bbcf'
             }else{
               var color = '#ee282c'
            ...
            // return (
                ...
                {this.state.user_id == item.executive_id && ((item.status == '1' ) || (item.status == '2')) ?
                  ...
                  this.state.canUnassigned == true && item.executive_id !== 0 ?
                    ...
                    <TouchableOpacity style={{top:0}} onPress={() =>context.props.navigation.navigate("UnassignedLeads",{view_lead:item})}> 
                    <View style={{borderRadius:35,borderColor:'#e5314e',width: 28, height: 27,borderWidth:1,justifyContent:'center',alignItems:'center'}}>
